...
public class Block {
    ...
    public List<Integer> limit(int index, int value) {
        ...
        for(int i = ((index < m_N) ? index : ((index - m_N) * m_N)); i < ((index < m_N) ? (index + m_N * m_N) : ((index - m_N) * m_N + m_N)); i += ((index < m_N) ? m_N : 1)) {
            if(m_GridList.get(i).limit(value)) next.add(i + m_Index * m_N * m_N);
        }
        return next;
    }
    ...
    public void expand(int index, int value) {
        ...
        for(int i = ((index < m_N) ? index : ((index - m_N) * m_N)); i < ((index < m_N) ? (index + m_N * m_N) : ((index - m_N) * m_N + m_N)); i += ((index < m_N) ? m_N : 1)) {
            if(m_ConstraintList.get((index < m_N) ? (i / m_N + m_N) : (i % m_N)).get(value) == null) m_GridList.get(i).expand(value);
        }
    }
    ...
    public List<Integer> choose(int index, int rand, boolean isValue) {
        ...
        for(int i = 0; i < m_N * m_N; i++) {
            if(i == index) continue;
            if(m_GridList.get(i).limit(isValue ? rand : value)) next.add(i + m_Index * m_N * m_N);
        }
        m_Count++;
        return next;
    }
